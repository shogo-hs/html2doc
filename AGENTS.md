...
- 実装コードは `src/html2doc` 配下にまとまっています。CLI エントリポイントは `src/html2doc/cli.py` です。
...
2. **構成の意識**
   - 既存のモジュール分割の責務境界を尊重し、役割が混在しないようにしてください。
...
## テストと検証
- Python コードに変更が及ぶ場合は、最低限 `uv run html2doc --help` が成功することを確認してください。
- 振る舞いに影響する変更を行った場合は、再現手順と期待結果を説明したうえで `uv run pytest` などのテストコマンドを実行してください。
- 追加の検証（例: `uv run html2doc <サンプル入力>`) を行った場合は、使用した引数と結果を報告してください。
